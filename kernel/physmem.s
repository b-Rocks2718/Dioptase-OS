...
physmem_metadata_init:
  mov  r4, r0 # i = 0
  add  r6, r0, 1 # r6 = 1
  add  r7, r0, 40 # r7 = sizeof(struct Page) = 40
...
physmem_metadata_init_loop:
  ...
  # init ref_cnt
  swa r0, [r5, 4] # physmem_map[i].ref_cnt = 0

  # init refs
  swa r0, [r5, 8] # physmem_map[i].refs = NULL

  # init cache_entry
  swa r0, [r5, 12] # physmem_map[i].cache_entry = NULL
  ...
  swa r0, [r5, 16] # physmem_map[i].lock.lock.the_lock = 0
  swa r0, [r5, 20] # physmem_map[i].lock.lock.interrupt_state = 0
  swa r6, [r5, 24] # physmem_map[i].lock.count = 1
  swa r0, [r5, 28] # physmem_map[i].lock.wait_queue.head = NULL
  swa r0, [r5, 32] # physmem_map[i].lock.wait_queue.tail = NULL
  swa r0, [r5, 36] # physmem_map[i].lock.wait_queue.size = 0
