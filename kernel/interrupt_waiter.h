...
/*
 * Single-thread waiter handoff for interrupt-driven devices.
 *
 * Concurrency contract:
 * - Exactly one thread may publish itself as the waiter.
 * - Device ISRs may signal from any core.
 ...
 *   and owned by the caller, which must enqueue exactly one deferred wake.
 *
 * CPU-state contract:
 * - init runs in kernel mode before the device ISR is enabled, or during
 *   shutdown after that ISR can no longer execute.
 * - prepare runs in the sole worker/playback thread in kernel mode. Interrupts
 ...
 * - signal runs in kernel ISR context with global interrupts disabled by
 *   hardware. It must not directly context switch or modify the TCB.
 *
 ...
 * device. An event concurrent with this store is safe: it is either represented
 * in the device/buffer being checked or remains pending for publish to consume.
...
/*
 * Publish one already-blocked thread. If an interrupt signalled before the
...
/*
 * Record a device event before looking for the waiter. Return and detach the
 ...
 * it; a non-NULL return is detached and owned exclusively by the ISR.
 */
