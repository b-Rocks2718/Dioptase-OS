 ...
 * - gate_destroy() succeeds only after every waiter has returned, which is the
 *   gate's required externally quiescent teardown point
 *
 * How:
 ...
 * - signal and join that final waiter set before destroying the gate
 */
...
void kernel_main(void) {
  ...
  // reset_done is published only after gate_wait() returns. Once all final
  // waiters have incremented it, no worker can touch either the condition
  // variable or the embedded blocking lock, satisfying quiescent teardown.
  gate_destroy(&gate);
