 ...
 *
 * Validates:
 * - slab_heap_alloc() and slab_heap_free() preserve payload bytes across randomized allocation and
 *   slab_heap_free patterns
 * - slab_heap_freeing in shuffled order does not corrupt neighboring live allocations
 ...
 * - slab_heap_free half the blocks in shuffled order after verifying their contents
 * - allocate and immediately churn a second batch to force reuse of slab_heap_freed holes
 * - verify and slab_heap_free the remaining original blocks, then repeat for several rounds
 */
...
// Shuffle an index array so slab_heap_frees happen in a different order every round.
...
// Run one heap stress workload with randomized sizes and slab_heap_free order.
static void heap_stress(unsigned rounds, unsigned N) {
  ...
  for (unsigned r = 0; r < rounds; r++) {
    ...
    for (unsigned i = 0; i < N; i++) {
      ...
      void* p = slab_heap_alloc(sz);
      if (!p) {
        int args[3] = {(int)r, (int)i, (int)sz};
        say("| slab_heap_alloc failed at round=%d i=%d sz=%d\n", args);
        panic("heap_stress: slab_heap_alloc returned NULL\n");
      }
    ...
    // Phase 2: slab_heap_free half the blocks in shuffled order after validating them.
    shuffle(order, N);
    ...
    for (unsigned k = 0; k < halfway; k++) {
      ...
      slab_heap_free(b->ptr);
      b->ptr = NULL;
    }

    // Phase 3: churn smaller blocks to force reuse of the slab_heap_freed holes.
    for (unsigned i = 0; i < halfway; i++) {
      unsigned roll = rnd_u32();
      unsigned sz = 16 + (roll % 256);
      void* p = slab_heap_alloc(sz);
      if (!p) panic("heap_stress: slab_heap_alloc failed during reuse phase\n");
    ...
    // Phase 4: verify and slab_heap_free the surviving original blocks.
