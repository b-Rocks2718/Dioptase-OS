 ...
 * - barrier_destroy() succeeds after the fixed participant set has returned,
 *   which is the barrier's required externally quiescent teardown point
 *
 * How:
 ...
 * - join the complete participant set before destroying the barrier
 */
...
void kernel_main(void) {
  ...
  // finished is published only after each worker's final barrier_sync() has
  // returned. No worker can touch the barrier after that publication, so the
  // complete count establishes the externally quiescent destroy precondition.
