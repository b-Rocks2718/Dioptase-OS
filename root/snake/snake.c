...
static int direction_between(int from_x, int from_y, int to_x, int to_y) {
  ...
  // non-adjacent body segment in tile selection
  exit(1);
  return DIR_UP;
}
...
static bool play_round(unsigned seed, unsigned* next_seed) {
  ...
  bool dirty = true;
  unsigned last_frame = get_vga_frame_counter();

  reset_game(&game, seed);
  *next_seed = seed * RNG_MULTIPLIER + RNG_INCREMENT + get_current_jiffies();
  ...
  while (!restart_requested && !quit_requested) {
    ...
    if (game.score != old_score) {
      if (game.score > snake_high_score) {
        snake_high_score = game.score;
      }
    }
...
int main(void) {
  ...
  // hide terminal cursor and clear screen before starting the game
  puts("\x1b[?25l\x1b[2J");
