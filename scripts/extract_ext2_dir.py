...
This recreates the emulator's SD1 image as ordinary files, directories, and
symlinks so OS tests can inspect filesystem side effects without a loop mount.
It relies on ``debugfs ls -p``, ``dump``, and ``stat``; paths passed to debugfs
...
# Execute one read-only debugfs command and normalize its mixed output streams.
...
# Join a name to an absolute guest path without introducing a double slash.
...
# Decode the inode, mode, and name fields from debugfs's slash-delimited output.
...
# Recover either a fast inline symlink or a block-backed target from the image.
...
# Extract the directory tree and files represented by the ext2 image.
...
# Validate the command line and recreate an ext2 image's tree on the host.
def main() -> int:
